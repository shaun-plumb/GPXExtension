/**
 * Content script - runs in the context of Google Maps pages
 */
...
chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
  if (request.action === 'getRouteData') {
    try {
      ...
      const routeInfo = extractRouteInfo(currentUrl);

      if (!routeInfo) {
        sendResponse({
          success: false,
          error: 'Could not extract route information from the URL'
        });
        return true;
      }

      // Check if there was an error (e.g., no start point selected)
      if (routeInfo.error) {
        sendResponse({
          success: false,
      ...
      sendResponse({
        ...
        locationNames: routeInfo.locationNames,
